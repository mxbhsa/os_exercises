...
>  首先需要已经完成段机制的建立，这时已经对GDT进行了初始化。在此基础上实现段机制，进而才能实现页内存管理。然后进行线性地址的映射转换，然后根据页表级数来建立页表目录，建立页表，在保护模式下设置CR0寄存器第31位（PG）进行使能页
...
因此我们可以推断, 打开中断导致的crash, 因为在代码`trap.c`中, 我们需要对中断向量表进行初始化,
没有初始化直接打开中断就会导致crash
...
trapframe at 0x7b5c
  ...
  fs   0x----0023
  gs   0x----0023
  trap 0x00000014 (unknown trap)
  err  0x00000000
  eip  0x00100070
  cs   0x----0008
  flag 0x00000207 CF,PF,IF,IOPL=0
kernel panic at kern/trap/trap.c:191:
